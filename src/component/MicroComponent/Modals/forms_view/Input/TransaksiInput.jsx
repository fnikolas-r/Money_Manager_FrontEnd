...
export default function TransaksiInput(props) {
    ...
    useEffect(() => {
        if (id) {
            if (detail.trc_name === "") {
                dispatch(get_detail_transaksi({id}))
            }
    ...
    const handle_transaksi = (data) => {
        console.log(data)
        if (id) {
            dispatch(edit_transaksi({
                ...data,
                trc_date: dayjs(data.trc_date).format("YYYY-MM-DDTHH:mm").toString(),
                id
            }))
        } else {
            ...
            <div className="flex mt-2">
                <div className="sm:col-span-3 grow mr-1.5">
                    ...
                    <div className="mt-1">
                        ...
                        />
                        {errors.trc_name && <span className="text-red-500 text-sm">{errors.trc_name?.message}</span>}
                    </div>
                </div>
                <div className="sm:col-span-3 grow">
                    ...
                    <div className="mt-1">
                        <input
                            ...
                            className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md"
                        />
                        {errors.pelaku && <span className="text-red-500 text-sm">{errors.pelaku?.message}</span>}
                    </div>
            ...
            <div className="sm:col-span-3 mt-3">
                ...
                <div className="mt-1">
                    <input
                        ...
                        className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md"
                    />
                    {errors.trc_date && <span className="text-red-500 text-sm">{errors.trc_date?.message}</span>}
                </div>
            </div>
            <div className="sm:col-span-3 mt-3">
                ...
                <div className="mt-1">
                    ...
                    {errors.rekening && <span className="text-red-500 text-sm">{errors.rekening?.message}</span>}
                </div>
            </div>
            <div className="flex mt-2">
                <div className="sm:col-span-3 mt-3 grow mr-1.5 ">
                    ...
                    <div className="mt-1">
                        ...
                        {errors.trc_type && <span className="text-red-500 text-sm">{errors.trc_type?.message}</span>}
                    </div>
                </div>
                <div className="sm:col-span-3 mt-3 grow">
                    ...
                    <div className="mt-1">
                        ...
                        {errors.kategori && <span className="text-red-500 text-sm">{errors.kategori?.message}</span>}
                    </div>
                </div>
            </div>
